...
### 5.4 早期试点 `pilots/`（保留作对照，见 D-013/D-015）

- `pilots/ws/` — 阶段三最早试点（openai-chat、youte-search），含 `enhanced-page.md`/`writeback.diff` 等旧路线产物。
- `pilots/phase3-pilot/` — 试点定型版，含独立的 `inline_inject.py`。
- 两者已被 `ws-v2/` 全量路线取代，但 `DECISIONS.md` 明令「旧试点产物保留作对照，不删」，故统一收拢到 `pilots/` 下。
...
1. **去重消歧**：核对无重复内容文件。`ai-friendly-prose` 技术版与 readable 版是同源不同受众的两份，非重复；`pilots/ws/` 与 `pilots/phase3-pilot/` 是两次试点快照，内容不同，均按 D-013/D-015 保留。
2. **清理 cruft**：删除全部 `__pycache__/` 与 `*.pyc`（本就 gitignored，5 处）。
3. **移除归档**：删除早期求职作品集阶段的 `archive/` 目录及其全部内容（已不代表当前方向，与文档质量工程无关）。
4. **收拢试点**：原散落在根的 `ws/`、`phase3-pilot/` 统一收拢到 `pilots/` 下。
5. **一致性**：本清单与 `CHARTER.md` 第五节资产地图、`STATE.md` 已完成项对齐。
6. **不动复现链**：根 `.py` 与被 import 的工具脚本一律原位保留（第六节）。

...
按关键词检索 YouTube 视频并返回结构化结果列表，底层经 SearchApi 聚合的 YouTube 搜索引擎完成。属只读查询接口，支持按地区、界面语言以及 YouTube 过滤令牌进一步收窄结果，适用于内容发现、竞品监测、素材采集等场景。鉴权采用 HTTP Bearer，需在 `Authorization` 头携带 AIsa 平台签发的 API Key。
## 请求说明
- 请求方式：**GET**
...
```
## 返回参数
成功时（HTTP `200`）返回搜索结果载荷，包含本次检索的元信息与命中的视频列表。
```mixin-react
const columns = [
  {
    "title": "参数名称",
...
const data = [
  {
    ...
    "key": "0-1-0",
    "title": "本次搜索的元信息，记录检索状态、查询参数回显等维度的附加数据，便于排查与审计。",
    "example": "",
    "type": "Object"
  },
  {
...
| 错误码 | HTTP 状态码 | 说明 |
| --- | --- | --- |
| 400 |  | 请求参数无效。常见于缺少必填的 `engine` 或 `q`、参数取值不合法。 |
| 401 |  | 鉴权失败。`Authorization` 头缺失，或 Bearer API Key 无效。 |

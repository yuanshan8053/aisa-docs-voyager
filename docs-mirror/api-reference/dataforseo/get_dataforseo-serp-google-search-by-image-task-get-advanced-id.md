...
````yaml openapi/dataforseo.json GET /dataforseo/serp/google/search_by_image/task_get/advanced/{id}
openapi: 3.0.3
info:
...
paths:
  /dataforseo/serp/google/search_by_image/task_get/advanced/{id}:
    get:
      ...
      description: >-
        Get Google Search By Image SERP Advanced Results by id. Results can be
        retrieved by task id within the retention window after posting the task.
      operationId: get_dataforseo_serp_google_search_by_image_task_get_advanced_id
      parameters:
        - name: id
          in: path
          required: true

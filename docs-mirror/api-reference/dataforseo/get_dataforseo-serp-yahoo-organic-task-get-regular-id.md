...
````yaml openapi/dataforseo.json GET /dataforseo/serp/yahoo/organic/task_get/regular/{id}
openapi: 3.0.3
info:
...
paths:
  /dataforseo/serp/yahoo/organic/task_get/regular/{id}:
    get:
      ...
      description: >-
        Get Yahoo Organic SERP Results by id (regular). Returns regular SERP
        results for a previously posted Yahoo Organic task.
      operationId: get_dataforseo_serp_yahoo_organic_task_get_regular_id
      parameters:
        - name: id
          in: path
          required: true

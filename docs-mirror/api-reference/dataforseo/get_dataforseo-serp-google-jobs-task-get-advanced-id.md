...
````yaml openapi/dataforseo.json GET /dataforseo/serp/google/jobs/task_get/advanced/{id}
openapi: 3.0.3
info:
...
paths:
  /dataforseo/serp/google/jobs/task_get/advanced/{id}:
    get:
      ...
      description: >-
        Get Google Jobs Advanced Results by id. Results can be retrieved by task
        id within the retention window after posting the task.
      operationId: get_dataforseo_serp_google_jobs_task_get_advanced_id
      parameters:
        - name: id
          in: path
          required: true
